...
const sirnamejustin="Justin Bieber"
fenomenler[6]["profile"]=sirnamejustin;
...
function indekseGoreFenomen(array,indeks) {
   return `${indeks}. indekste bulunan fenomen: ${array[indeks]["profile"]}`
}
...
function fenomenSil(array,indeks) {
  let copyArray1=array.slice();
  copyArray1.splice(indeks,1);
  return copyArray1;
}
...
function fenomenEkle(array,number,profile,followers,posts,platform) {
 ...
 copyArray2.push(user);
 return copyArray2;
}
console.log(fenomenEkle(fenomenler,7,"atstech",250000,2023,"www.ats.com"));
...
function fenomenGonderimSayisi(array,alinanProfile){
  let endArray=[];
  let sonuc;
  for(let key in array){
    endArray[key]=array[key];
      if(endArray[key][alinanProfile]=array[key]["profile"]){
      sonuc= endArray[key]["posts"];
    }
  }
  return sonuc;
}
...
for(let i=0;i<array.length;i++){
  if(array[i].platform===platformName){
    if(array[i].posts>maxpost){
      maxpost=array[i].posts;
      result=array[i].profile;
    }
  }
}
return result;
